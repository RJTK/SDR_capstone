...
\begin{equation}
\label{eq:quadrature_sampling2}
A(t)cos(\omega t + \phi (t)) = A(t)cos(\omega t)cos(\phi (t)) - 
A(t)sin(\omega t)sin(\phi(t))
\end{equation}

Then we define $I(t) = A(t)cos(\phi (t))$ and $Q(t) = A(t)sin(\phi (t))$ to arrive at
...
To be clear, this gives us a way to represent the general sinusoid in equation \ref{eq:quadrature_sampling} by using a sum of a sine and a cosine.  This is called the I and Q components.  To recover the original representation, simply apply the definitions of $I$ and $Q$.
...
Finally, formula \label{eq:quadrature_sampling4} can be interpreted geometrically (fiure \ref{fig:quadrature_sampling}, and it becomes very convenient to use complex numbers to represent I/Q components.  That is, write the separate IQ components as a single complex number $I(t) + jQ(t)$.
...
\centering
\includegraphics[width=9cm]{images/IQ_sample.eps}
\end{figure}

Since all of these quantities ($A, I, Q, \phi$) are time varying, they can be visualized as a helix in 3 dimensions.  This representation is far more enlightening than the typical 2D plots most of us are used to seeing.  For example, AM modulation can be seen with an IQ representation in figure \ref{fig:am_iq}
